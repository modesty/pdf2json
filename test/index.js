...
testSuite(PdfParser_mjs);

function testSuite(PFParser) {
   ...
   function checkResult_parseStatus(err, stat, fileName) {
      ...
      assert(
         _.isObject(stat),
         fileName + ' parsing result should be JS object'
      ); // We have a stat object
   }

   function checkResult_mainFields(parsedData, fileName) {
      assert(
         _.has(parsedData, 'Agency'),
         fileName + " parsing error: doesn't have Agency object"
      );
      assert(
         _.has(parsedData, 'Id'),
         fileName + " parsing error: doesn't have Id object"
      );
      assert(
         _.has(parsedData, 'Pages'),
         fileName + " parsing error: doesn't have Pages object"
      );
      assert(
         _.has(parsedData, 'Width'),
         fileName + " parsing error: doesn't have Width object"
      );
   }
   ...
   function checkResult_pageCount(Pages, count, fileName) {
      ...
      assert(
         Pages.length === count,
         fileName + ' parsing error: Pages array is empty'
      );
   }
