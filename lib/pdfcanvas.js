...
(function () {
    ...
    contextPrototype.drawImage = function(image, sx, sy, swidth, sheight, x, y, width, height) {
		if (sx === undefined) {
			sx = 0;
		}
		if (sy === undefined) {
		...
		if (image instanceof CanvasRenderingContext2D_) {
		  ...
		  let scaleX = this.arcScaleX_ * canvasWidth / width, scaleY = this.arcScaleY_ * canvasHeight / height;
			console.log("copy");
		  image.canvas.Images.forEach(function(data) {
			let oneImage = {
			  ...
			  crop: data.crop,
	  		  display: {
				  x: data.display.x * scaleX + x,
				  y: data.display.y * scaleY + y,
				  width: data.display.width * scaleX,
				  height: data.display.height * scaleY
			  },
			  width: data.width,
			  height: data.height
			};
			if (data.crop.crop && crop) {
				data.crop = true;
				data.crop.x = data.crop.x * scaleX + x,
				data.crop.y = data.crop.y * scaleY + y,
				data.crop.width = data.crop.width * scaleX,
				data.crop.height = data.crop.height * scaleY
			}
			this.canvas.Images.push(oneImage)
		  }, this);
      } else {
        let oneImage = {
          image: image,
    ...
    contextPrototype.putImageData = function(imgData, x, y, width, height) {
		if (x === undefined) {
			x = 0;
		}
		if (y === undefined) {
