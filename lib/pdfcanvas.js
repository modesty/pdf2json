...
(function () {
     ...
     * be associated with
     */
    function CanvasRenderingContext2D_(canvasTarget, scaledWidth, scaledHeight, canvasScale) {
        // private
        var _id = _nextId++;
    ...
    contextPrototype.fillText = function(text, x, y, maxWidth, fontSize, wordSpacing, spaceWidth) {
        if (!text || text.trim().length < 1) {
            return;
        }
        var p = this.getCoords_(x, y);
        //apply the same canvas scaling to the maxWidth and wordSpacing as well
        maxWidth *= this.canvasScale;
        wordSpacing *= this.canvasScale;
        spaceWidth *= this.canvasScale;
        
        var a = processStyle(this.fillStyle || this.strokeStyle);
        var color = (!!a) ? a.color : '#000000';

        this.currentFont.processText(p, text, maxWidth, color, fontSize, this.canvas, this.m_, wordSpacing, spaceWidth);
    };

    contextPrototype.strokeText = function(text, x, y, maxWidth, wordSpacing, spaceWidth) {
        //MQZ. 10/23/2012, yeah, no hollow text for now
        this.fillText(text, x, y, maxWidth, null, wordSpacing, spaceWidth);
    };
