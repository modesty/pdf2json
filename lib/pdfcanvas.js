...
export default class CanvasRenderingContext2D_ {
	...
	// Add appendText method to handle text from the textLayer
	appendText(textGeom) {
		if (!textGeom) {
			nodeUtil.p2jinfo(`PDFCanvas.appendText: No text geometry provided`);
			return;
		}
		
		// Create a common function to generate text objects
		const createTextObject = (text, x, y, width, fontSize, isType3) => {
			// Ensure coordinates are rounded to 2 decimal places
			...
			const textInfo = {
				...
				// TT: isType3 ? 1 : 0 // Always include TT flag, 1 for Type3, 0 for regular
			};
			
			nodeUtil.p2jinfo(`PDFCanvas.appendText: Created text object at (${roundedX}, ${roundedY}): ${text}`);
			return textInfo;
		};
		
		// Handle Type3 text if available
		if (textGeom.type3Text) {
			// Apply text matrix for better coordinate calculation if available
			const { x: baseX = textGeom.x, y: baseY = textGeom.y } = textGeom;
			let x = baseX;
			let y = baseY;
			
			// Apply transform to get proper coordinates if textMatrix is available
			if (textGeom.textMatrix) {
				x = textGeom.textMatrix[4] || x;
				y = textGeom.textMatrix[5] || y;
				
				nodeUtil.p2jinfo(`PDFCanvas.appendText: Using text matrix for Type3: [${textGeom.textMatrix.join(', ')}]`);
			}
			
			nodeUtil.p2jinfo(`PDFCanvas.appendText: Processing Type3 text: '${textGeom.type3Text}' at (${x}, ${y})`);
			
			// Create and add the Type3 text object
			const textInfo = createTextObject(
				textGeom.type3Text, 
				x, 
				y, 
				textGeom.canvasWidth, 
				textGeom.fontSize, 
				true
			);
			
			nodeUtil.p2jinfo(`PDFCanvas.appendText: Adding Type3 text to output: ${JSON.stringify(textInfo)}`);
			
			// Add to the canvas Texts array - using unshift to make it appear at the beginning
			// which helps with layer order in case there are overlapping texts
			this.canvas.Texts.unshift(textInfo);
			return;
		}
		
		// Check for extracted Type3 texts in commonObjs
		if (textGeom.commonObjs && textGeom.commonObjs._extractedType3Texts) {
			const extractedTexts = textGeom.commonObjs._extractedType3Texts;
			if (extractedTexts.length > 0) {
				nodeUtil.p2jinfo(`PDFCanvas.appendText: Found ${extractedTexts.length} extracted Type3 texts`);
				
				// Process each extracted Type3 text
				for (const extractedText of extractedTexts) {
					const textInfo = createTextObject(
						extractedText.text,
						extractedText.x,
						extractedText.y,
						0, // No width info available
						extractedText.fontSize || 10,
						true // Type3 text
					);
					
					if (typeof nodeUtil !== 'undefined' && nodeUtil.p2jinfo) {
						nodeUtil.p2jinfo(`PDFCanvas.appendText: Adding extracted Type3 text to output: ${JSON.stringify(textInfo)}`);
					}
					
					// Add to the canvas Texts array
					this.canvas.Texts.unshift(textInfo);
				}
				
				// Clear the extracted texts to avoid duplicates
				textGeom.commonObjs._extractedType3Texts = [];
				return;
			}
		}
		
		// Process regular text
		if (textGeom.text || (textGeom.str && textGeom.str.length > 0)) {
			const text = textGeom.text || textGeom.str;
			
			if (typeof nodeUtil !== 'undefined' && nodeUtil.p2jinfo) {
				nodeUtil.p2jinfo(`PDFCanvas.appendText: Processing regular text: '${text}' at (${textGeom.x}, ${textGeom.y})`);
			}
			
			// Create and add the regular text object
			const textInfo = createTextObject(
				text,
				textGeom.x,
				textGeom.y,
			...
			if (typeof nodeUtil !== 'undefined' && nodeUtil.p2jinfo) {
				nodeUtil.p2jinfo(`PDFCanvas.appendText: Adding regular text to output: ${JSON.stringify(textInfo)}`);
			}
			
			// Add to the canvas Texts array
			this.canvas.Texts.push(textInfo);
		} else {
			if (typeof nodeUtil !== 'undefined' && nodeUtil.p2jinfo) {
				nodeUtil.p2jinfo(`PDFCanvas.appendText: No text content found in geometry object: ${JSON.stringify(textGeom)}`);
			}
		}
	}
...
export function createScratchCanvas(width, height) {
	// Create a canvas object with storage for Type3 text to ensure it's captured
	const canvas = {
		HLines: [],
		VLines: [],
		Fills: [],
		Texts: [],
		_extractedType3Texts: []
	};
	
	return new CanvasRenderingContext2D_(canvas, width, height);
}
