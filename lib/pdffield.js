...
let PDFField = (function PDFFieldClosure() {
    ...
    cls.getAllFieldsTypes = function(data) {
        ...
        let pagesCount = data.Pages.length;

        function isFieldReadOnly(field) {
            return (field.AM & kFBANotOverridable) ? true : false;
        }

        function getFieldBase(field, page) {
            return {id: field.id.Id, type: field.T.Name, calc: isFieldReadOnly(field), value: field.V || "", x: field.x, y: field.y, page: page};
        }
            ...
            _.each(page.Boxsets, function(boxsets) {
                if (boxsets.boxes.length > 1) { //radio button
                    _.each(boxsets.boxes, function(box) {
                        retVal.push({id: boxsets.id.Id, type: "radio", calc: isFieldReadOnly(box), value: box.id.Id, x: box.x, y: box.y, page: currentPage});
                    });
                }
                else { //checkbox
                    retVal.push(getFieldBase(boxsets.boxes[0], currentPage));
                }
            });

            _.each(page.Fields, function(field){
                retVal.push(getFieldBase(field, currentPage));
            });
        });
        return retVal;
    };
