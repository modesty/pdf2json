...
var optimist = require('optimist')
    ...
    .alias('e', 'selector')
    .describe('e', '(optional) when specified, will extract a json file containing only fields for the selector in css mode.\n');
...
var PDF2JSONUtil = (function () {
    ...
    var _writeOneJSON = function(data, callback) {
        ...
        if(_.has(argv,'e')){
            var content_parsed = JSON.parse(pJSON);
            var json_selected = JSONSelect.match(argv.e,content_parsed);
            fs.writeFile(this.outputPath,JSON.stringify(json_selected,null,4),function(err) {
                if(err){
