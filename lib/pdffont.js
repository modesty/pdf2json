...
let PDFFont = (function PFPFontClosure() {
    ...
    let cls = function (fontObj) {
        ...
        if (this.fontObj.isSymbolicFont) {
            let mFonts = _.filter(_stdFonts, function(oneName){
	    ...
	    this.spaceWidth = PDFUnit.toFormX(this.spaceWidth);
    };
    ...
    cls.getSpaceThreshHold = function(t1) {
        return (PDFFont.getFontSize(t1) - 11) * t1.sw * 0.063;
    };

    cls.areAdjacentBlocks = function(t1, t2) {
        let isInSameLine = Math.abs(t1.y - t2.y) <= DISTANCE_DELTA;
        let isDistanceSmallerThanASpace = (Math.abs(t2.x - t1.x - t1.w) < cls.getSpaceThreshHold(t1));
