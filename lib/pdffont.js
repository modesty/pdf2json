...
var PDFFont = (function PFPFontClosure() {
    ...
    var _stdFonts = ["arial", "sans-serif ", "courier ","monospace ", "ocr "];
    ...
    var cls = function (fontObj) {
        ...
        if (this.fontObj.isSymbolicFont) {
            ...
            if (mFonts.length > 0) {
                ...
                nodeUtil.p2jinfo("Reset: isSymbolicFont (false) for " + typeName);
            }
        }
        else {
            if (this.isSymbol)
...
//            p.y -= this.fontSize * 0.375;
        ...
        var oneText = {x: PDFUnit.toFormX(p.x) - 0.25,
            ...
            // calculate PostScript point coordinates for x and y (where [0, 0] is in the bottom left corner)
            pointX: p.x / targetData.viewportInfo.width * targetData.viewportInfo.pageView[2],
            pointY: targetData.viewportInfo.pageView[3] - targetData.viewportInfo.pageView[3] * p.y / targetData.viewportInfo.height,
            w: maxWidth,
            clr: clrId,
            A: "left",
            R: [{
