...
export default class PDFFont {
   ...
   #processType3Font(str) {
      ...
      // Debug info
      nodeUtil.p2jinfo(`Processing Type3 font: char code = ${str.charCodeAt(0)}, char = '${str}'`);
      if (this.fontObj.charProcMapping) {
         nodeUtil.p2jinfo(`charProcMapping available with ${Object.keys(this.fontObj.charProcMapping).length} entries`);
      } else {
         nodeUtil.p2jinfo(`No charProcMapping available for this Type3 font`);
         
         // If no mapping is available, try to use the character directly
         if (str && str.length === 1) {
            const code = str.charCodeAt(0);
            if (code >= 65 && code <= 90) { // A-Z
               nodeUtil.p2jinfo(`Using direct uppercase letter: ${str}`);
               return str;
            } else if (code >= 97 && code <= 122) { // a-z
               nodeUtil.p2jinfo(`Using direct lowercase letter: ${str}`);
               return str;
            } else if (code >= 48 && code <= 57) { // 0-9
               nodeUtil.p2jinfo(`Using direct digit: ${str}`);
               return str;
            }
         }
      }
      ...
      if (this.fontObj.charProcMapping) {
         ...
         if (glyphName) {
            nodeUtil.p2jinfo(`Found glyph name in mapping: ${glyphName}`);
            ...
            if (glyphNameLower in glyphToUnicode) {
               ...
               nodeUtil.p2jinfo(`Mapped ${glyphNameLower} to Unicode ${unicodeChar}`);
               return unicodeChar;
            }
            ...
            if (typeof glyphName === 'string' && glyphName.length > 1) {
               ...
               if (letterMatch && letterMatch.length === 1) {
                  ...
                  nodeUtil.p2jinfo(`Extracted letter ${letter} from glyph name ${glyphName}`);
                  return letter;
               }
            }
            ...
            for (const key in glyphToUnicode) {
               if (glyphNameLower.indexOf(key) >= 0) {
                  ...
                  nodeUtil.p2jinfo(`Partial match: ${glyphNameLower} contains ${key}, mapped to ${unicodeChar}`);
                  return unicodeChar;
               }
            }
            ...
            if (typeof glyphName === 'string') {
               ...
               if (numberMatch && numberMatch.length === 1) {
                  ...
                  if (num >= 1 && num <= 26) {
                     ...
                     nodeUtil.p2jinfo(`Mapped number ${num} in glyph name ${glyphName} to letter ${letter}`);
                     return letter;
                  }
               }
            }
            ...
            if (typeof glyphName === 'string' && glyphName.startsWith('uni')) {
               ...
               if (/^[0-9A-F]{4,6}$/i.test(hex)) {
                  nodeUtil.p2jinfo(`Mapped uni${hex} to Unicode character`);
      ...
      nodeUtil.p2jinfo(`No direct mapping for character code ${charCode}, checking general mappings`);
      ...
      if (result !== str) {
         nodeUtil.p2jinfo(`Mapped char code ${charCode} to Unicode ${result}`);
      } else {
         nodeUtil.p2jinfo(`No mapping found for char code ${charCode}, returning original character`);
      }
   ...
   processText(p, str, maxWidth, color, fontSize, targetData, matrix2D) {
      ...
      nodeUtil.p2jinfo(`Processing text: '${str}', font type: ${this.fontObj.type || 'unknown'}, char code: ${str ? str.charCodeAt(0) : 'none'}`);
      ...
      if (!text) {
         nodeUtil.p2jinfo('Text processing returned null or empty, falling back to original text');
         text = originalStr; // Use original text as fallback
      }
      
      nodeUtil.p2jinfo(`Processed text: '${str}' -> '${text}'`);
      ...
      nodeUtil.p2jinfo(`Adding text to output: '${text}'`);
