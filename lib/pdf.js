...
class PDFPageParser{
    ...
    parsePage(callback, errorCallBack) {
        ...
        function pageViewDrawCallback(error) {
            ...
            if (error) {
                errorCallBack('An error occurred while rendering the page ' + (this.id + 1) +
                    ':\n' + error.message +
                    ':\n' + error.stack );
            }
            else {
                if (this.ptiParser) {
                    ...
                    _.each(extraFields, _.bind(this.#_addField, this));
                }
        ...
        this.pdfPage.render(renderContext).then(
            data => {
                this.pdfPage.getAnnotations().then(
                    fields => {
                        _.each(fields, _.bind(this.#_addField, this));
...
class PDFJSClass extends EventEmitter {
    static _getMetaDataString(metadata, key){
        let retVal = "unknown";
    ...
    static _getMetaDataInt(metadata, key){
        let retVal = PDFJSClass._getMetaDataString(metadata, key);
    ...
    parseMetaData() {
        ...
        const formAttr = {AgencyId:"", Name: "", MC: false, Max: 1, Parent:""};
        if (metadata) {
            formAttr.AgencyId = PDFJSClass._getMetaDataString(metadata, 'pdfx:agencyid');
            if (formAttr.AgencyId != "unknown")
                pdfTile = formAttr.AgencyId;

            formAttr.Name = PDFJSClass._getMetaDataString(metadata, 'pdfx:name');
            formAttr.MC = PDFJSClass._getMetaDataString(metadata, 'pdfx:mc') === 'true';
            formAttr.Max = PDFJSClass._getMetaDataInt(metadata, 'pdfx:max');
            formAttr.Parent = PDFJSClass._getMetaDataInt(metadata, 'pdfx:parent');
        }
