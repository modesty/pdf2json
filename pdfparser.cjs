   ...
   processLine(targetData) {
      ...
      if (yDelta < this.lineWidth && xDelta > minDelta) {
         ...
         if (this.lineWidth < 4 && xDelta / this.lineWidth < 4) {
            nodeUtil.p2jinfo(
               'Skipped: short thick HLine: lineWidth = ' +
                  this.lineWidth +
                  ', xDelta = ' +
                  xDelta
            );
         ...
         if (this.lineWidth < 4 && yDelta / this.lineWidth < 4) {
            nodeUtil.p2jinfo(
               'Skipped: short thick VLine: lineWidth = ' +
                  this.lineWidth +
                  ', yDelta = ' +
                  yDelta
            );
...
class PDFFill {
   ...
   processFill(targetData) {
      ...
      if (oneFill.w < 2 && oneFill.h < 2) {
         nodeUtil.p2jinfo(
            'Skipped: tiny fill: ' + oneFill.w + ' x ' + oneFill.h
         );
         return; //skip short thick lines, like PA SPP lines behinds checkbox
      }
...
class PDFField {
   ...
   static isWidgetSupported(field) {
      ...
      switch (field.fieldType) {
         ...
         default:
            nodeUtil.p2jwarn(
               'Unsupported: field.fieldType of ' + field.fieldType
            );
            break;
      }
...
const __filename$2 = url.fileURLToPath(
   typeof document === 'undefined'
      ...
      : (_documentCurrentScript && _documentCurrentScript.src) ||
           new URL('pdfparser.cjs', document.baseURI).href
);
const __dirname$2 = path.dirname(__filename$2);

const pkInfo = JSON.parse(
   fs.readFileSync(`${__dirname$2}/package.json`, 'utf8')
);
...
class PDFParser extends events.EventEmitter {
   ...
   async loadPDF(pdfFilePath, verbosity) {
      ...
      try {
         ...
         PDFParser.#binBuffer[this.binBufferKey] = await promises.readFile(
            pdfFilePath
         );
   ...
   getMergedTextBlocksStream() {
      return ParserStream.createContentStream(
         this.getMergedTextBlocksIfNeeded()
      );
   }
